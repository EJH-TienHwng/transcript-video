...
def is_bad_hallucination_text(text: str) -> bool:
	...
	if not normalized:
		return True
	return any(phrase_norm and phrase_norm in normalized for phrase in BAD_PHRASES for phrase_norm in [normalize_text_for_filter(phrase)])


def remove_repeated_hallucination_segments(segments: Iterable[SubtitleSegment], max_same_text_count: int = 3, short_segment_seconds: float = 4.0) -> List[SubtitleSegment]:
	cleaned, repeat_count_by_text = [], {}
	for segment in segments:
		...
		if duration <= short_segment_seconds and repeat_count_by_text[normalized] > max_same_text_count:
			logging.warning("Removed repeated hallucination: %.2f --> %.2f | %s", segment.start, segment.end, text)
			continue
...
def fix_too_short_or_invalid_timing(segments: Iterable[SubtitleSegment]) -> List[SubtitleSegment]:
	...
	for segment in sorted(segments, key=lambda item: item.start):
		if segment.end <= segment.start:
			logging.warning("Removed invalid timing: %.2f --> %.2f | %s", segment.start, segment.end, segment.text)
			continue
		start, end = max(segment.start, last_end), max(segment.end, max(segment.start, last_end) + 0.2)
		last_end = end
...
def post_process_segments(segments: Iterable[SubtitleSegment]) -> List[SubtitleSegment]:
	...
	for segment in segments:
		...
		if is_bad_hallucination_text(text):
			logging.warning("Removed known hallucination: %.2f --> %.2f | %s", segment.start, segment.end, text)
			continue
...
def read_srt(srt_path: Path) -> List[SubtitleSegment]:
	...
	for block in re.split(r"\n\s*\n", content.strip()):
		...
		if text:
			segments.append(SubtitleSegment(parse_srt_timestamp(start_text), parse_srt_timestamp(end_text), text))
	return segments